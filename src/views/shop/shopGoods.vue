    ...
    <el-table :data="this.goods.slice((currentPage-1)*pagesize,currentPage*pagesize)" width="100%">
      ...
      <el-table-column prop="id" label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="danger" size="small" @click="down(scope.row)" plain>下架</el-button>
        </template>
...
export default {
  ...
  methods: {
    ...mapActions(['getGoodsByShopID', 'GoodsDown']),
    getGoods () {
      this.getGoodsByShopID(getShopId())
    },
    indexMethod (index) {
      return index + 1
    },
    ...
    down (row) {
      this.$confirm('此操作会将该上品下架, 是否继续?', '提示', {
        ...
        this.GoodsDown(row.goods.id).then(() => {
          this.$message({
            type: 'success',
            message: '已下架!'
          })
